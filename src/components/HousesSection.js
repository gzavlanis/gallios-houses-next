...
function HouseItem({ title, img, guests, size, desc, link, align }) {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: isLeft ? 'row' : 'row-reverse', alignItems: 'center', flexWrap: 'wrap' }}>
            <div style={{ flex: '0 0 60%', height: '500px', position: 'relative', overflow: 'hidden' }} className="house-img-wrap">
                <img src={img} alt={title} className="hover-zoom" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </div>
            <div style={{ flex: '0 0 50%', marginLeft: isLeft ? '-10%' : '0', marginRight: isLeft ? '0' : '-10%', background: 'white', padding: '60px', boxShadow: '0 30px 60px rgba(0,0,0,0.08)', position: 'relative', zIndex: 10 }} className="house-content-wrap">
            ...
            <style jsx>{`@media (max-width: 992px) { .house-img-wrap { flex: 0 0 100% !important; height: 350px; } .house-content-wrap { flex: 0 0 90% !important; margin: -50px auto 0 !important; } }`}</style>
        </div>
    );
}